import type { ChatBufferItem } from './chat-buffer-item';

export class ChatBuffer {
  private readonly bufferTable: Map<string, ChatBufferItem[]> = new Map();

  append(channelId: string, message: ChatBufferItem) {
    ...
    if (buffer.length > 0) {
      ...
      // 하나로 합칠 수 있는 메시지는 합침.
      if (latestMsg.authorId === message.authorId && elapsedTime < 60 * 1000) {
  ...
  flush(channelId: string): ChatBufferItem[] {
